...
export default function FeedMonitoringTable({ refreshKey = 0 }) {
  ...
  const groupedByFarmer = useMemo(() => {
    const groups = {};
    rows.forEach((row) => {
      if (!groups[row.farmer]) groups[row.farmer] = [];
  ...
  // 8 cols: Farmer | Brand | Type | Consumed | Allocated | Allocation Remaining | Purchase Remaining | Date
  const grid =
    "grid grid-cols-[1.2fr_1fr_1.1fr_0.9fr_0.9fr_1.2fr_1.2fr_1fr] gap-x-4 items-center";

  return (
    <div className="mt-6 rounded-lg bg-white border border-gray-300 shadow-sm">
      {/* Header with title */}
      <div className="px-6 pt-5 pb-4 border-b border-gray-200">
        <h2 className="text-[22px] font-bold text-gray-700">Feed Allocation History</h2>
        <p className="text-sm text-gray-500 mt-1">View all feed allocations to farmers</p>
      </div>
      ...
      <div className={`px-6 py-4 ${grid} bg-gray-50`}>
        ...
        <div className={`${header} text-center`}>Consumed (kg)</div>
        <div className={`${header} text-center`}>Allocated (kg)</div>
        <div className={`${header} text-center`}>Allocation Remaining (kg)</div>
        <div className={`${header} text-center`}>Purchase Remaining (kg)</div>
      ...
      <div className="px-0">
        {loading && (
          <div className="px-6 py-8 text-sm text-gray-500 text-center">Loading allocations…</div>
        )}
        ...
        {!loading && !err &&
          Object.entries(groupedByFarmer).map(([farmer, farmerRows], i) => (
            <div key={farmer} className={i > 0 ? "border-t border-gray-200" : ""}>
              {farmerRows.map((r, index) => {
                const isFirstInGroup = index === 0;
                const isAllocDepleted = Number(r.remainingKg) === 0;
                ...
                return (
                  ...
                  >
                    ...
                    {/* Consumed (kg) */}
                    <div className={`${cell} text-center font-semibold`}>{fmtKg(r.consumedKg)}</div>

                    {/* Allocated (kg) */}
                    <div className={`${cell} text-center font-semibold`}>{fmtKg(r.allocatedKg)}</div>

                    {/* Allocation Remaining (kg) */}
                    <div
                      className={`${cell} text-center font-semibold ${
                        isAllocDepleted ? "text-red-500" : "text-gray-700"
                      }`}
                    >
                      {fmtKg(r.remainingKg)}
                      {isAllocDepleted && (
                    ...
                    {/* Purchase Remaining (kg) */}
                    <div className={`${cell} text-center`}>{fmtKg(r.purchaseRemainingKg)}</div>
      ...
      {!loading && !err && rows.length > 0 && (
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div className="text-sm text-gray-600">
            ...
            {" • "}
            Total farmers:{" "}
            <span className="font-semibold">{Object.keys(groupedByFarmer).length}</span>
          </div>
        </div>
      )}
    </div>
